...
const QuizShow = (props) => {
    ...
    for (let i = 0; i < cols.length; i++) {
        ...
        for (let k = 0; k < fields.length; k++){
                if(fields[k].categorie_name == cols[i]){
                    rows.push(<PlayingField points = {fields[k].point} id ={fields[k].question.id} quizid= {quizId} status1={status[k]} position={k} categorie={fields[k].categorie_name}/>)
                }
                //else{
    ...
    return (
        <div className="container">
            <p style={{textAlign: "center",fontSize:'20px', fontWeight:"bold"}}>{quiz_name}</p>
            <div className='d-sm-flex justify-content-center'>{cats}</div>
            <div className='d-sm-flex justify-content-center'>
                {myFields}
            </div>
            <button className="btn btn-secondary my-4 float-end" onClick={() => navigate(-1)}> End Quiz</button>
